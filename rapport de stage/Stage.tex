...
\item Faire en sorte que la consommation de puissance du device cryptographique soit \textbf{\textit{identique}}. Cela signifie qu'à chaque coup de clock, une quantité égale de puissance est consommée pour toutes les opérations exécutées et pour toutes les données manipulées. Le but est donc de \textbf{modifier l'intensité du leakage}. Autrement dit, on souhaite uniformiser l'amplitude de la trace. Pour ce faire, on va à nouveau modifier le SNR. Comme dit précédemment, l'idéal serait d'avoir un SNR proche de 0. Si on n'augmente pas la variance du bruit, la deuxième approche consiste à diminuer la variance du signal. En pratique, il existe deux possibilités pour diminuer la variance du signal :
\begin{itemize}
\item La première approche concerne la cellule logique CMOS en elle-même. On sait (section \ref{sec:puissance}) que la consommation de puissance totale d'un device cryptographique est la somme des puissances consommées par chaque cellule. Ainsi, si chaque cellule consomme une puissance constante, la puissance totale est constante. Il faut donc construire des cellules qui consomment des quantités de puissance constantes.
...
\hspace{-0.5 cm}La figure \ref{fig:hiding} ci-dessous résume les trois possibilités de contre-mesures de type Hiding, à savoir : 
\begin{enumerate}
\item La consommation de puissance du device cryptographique est rendue aléatoire en changeant aléatoirement la séquence des opérations exécutées. Cette approche conduit à un désalignement des traces : seul l'instant du leakage est affecté. 
\item La consommation de puissance du device cryptographique est rendue aléatoire en réalisant des opérations de commutation en parallèle de l'exécution de l'opération actuelle de l'algoritme. Cela conduit à une augmentation de la variance du bruit : seul d'intensité du leakage est affectée.
\item  La consommation de puissance du device cryptographique est égale durant chaque coup de clock. Cela signifie que la variance du signal est diminuée : seul l'intensité du leakage est affectée.
\end{enumerate}

\begin{figure}[htbp]
    \centering
    \includegraphics[scale=0.4]{image/hiding}
