...
\begin{itemize}
\item \textbf{En modifiant \textit{l'amplitude} de la trace.}
\item \textbf{En modifiant \textit{la position dans le temps} de la trace.}
\end{itemize}



\subsubsection{Contre-mesures \textit{Hiding}}
Le principe des contre-mesures de type Hiding est de rendre la consommation de puissance du device cryptographique indépendante des opérations exécutées et des données manipulées. De cette façon, deux approches sont possibles :
\begin{itemize}
\item Faire en sorte que la consommation de puissance du device est \textbf{\textit{aléatoire}}. Cela signifie qu'à chaque coup de clock, une certaine quantité de puissance est consommée indépendamment des opérations exécutées et des données manipulées. Le but est donc de \textbf{modifier l'instant du leakage}. De cette façon, on cause un désalignement des traces, compliquant ainsi la tâche de l’attaquant. En effet, celui-ci doit, dans un premier temps, procéder à un alignement de ses traces pour pouvoir les analyser. Ce désalignement des traces peut s’obtenir de différentes façons : utiliser des horloges de fréquences différentes, utiliser des interruptions aléatoires lors de l’exécution du programme, changer l’ordre des instructions, etc.
\item Faire en sorte que la consommation de puissance du device est \textbf{\textit{identique}}. Cela signifie qu'à chaque coup de clock, une quantité égale de puissance est consommée pour toutes les opérations exécutées et pour toutes les données manipulées. Le but est donc de \textbf{modifier l'intensité du leakage}. Un exemple simple pour modifier l’intensité du leakage est de modifier le rapport du signal au bruit (SNR : Signal Noise Ration en anglais). On va ajouter du bruit de façon indépendante à l'exécution de l'algorithme de chiffrement. Ainsi, en fonction de la modification du SNR, l'attaquant aura plus de mal à retrouver la clé de secrète.
\end{itemize}
...
"la consommation de puissance d'un device cryptographique est indépendant des opération exécutées et des données manipulées SSI une des deux propriétés suivantes est observées :
\begin{itemize}
\item Le device cryptographique consomme aléatoirement de la puissance à chaque coup de clock.
\item Le device cryptographique consomme équitablement de la puissance à chaque coup de clock.
\end{itemize}
